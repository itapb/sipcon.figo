...
namespace Data
{
    public class dMaster
    {
        ...
        #region TASA
        public async Task<Response<Models.Result>> PostAlterRate(List<Models.Rate> _list)
        {
            await _semaphore.WaitAsync(Util.Setting.TimeOut);
            try
            {
                return await _PostAlterRate(_list);
            }
            finally
            {
                _semaphore.Release();
        ...
        private async Task<Response<Models.Result>> _PostAlterRate(List<Models.Rate> _list)
        {
            Response<Models.Result> _response = new Response<Models.Result>();
            try
            {
                ...
                _mapping.SetDefaultPostMapping();
                Util.Data _data = Util.Data.GetInstance();
                DataTable _table = await _data.GetDataTable("USP_POST_RATE_FIGO", _parameter);
                _response.Data = _data.GetItem<Models.Result>(_mapping, _table);
                _response.SetPostResponse();
            }
            catch (Exception ex)
